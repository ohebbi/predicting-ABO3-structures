...
import os
if not os.path.exists('data/data_extraction/AFLOW'):
    os.makedirs('data/data_extraction/AFLOW')
...
def dataMiningCandidates():
    ...
    keys = list(pd.read_csv("data/data_extraction/AFLOW/AFLOW_keywords.txt", sep=",").columns)
    aflow_dict = {k: [] for k in keys}

    AFLOW_entries = get_dataframe_AFLOW(compound_list=compound_list, keys=keys, batch_size=1000, catalog="icsd",fileName="data/data_extraction/AFLOW/AFLOW_data.csv")

    #writing to csv
    AFLOW_entries.to_csv("data/data_extraction/AFLOW/AFLOW_data.csv", sep=",", index = False)
...
def entireMP():
    ...
    #choosing keys used in AFLOW. We will here use all features in AFLOW.
    keys = list(pd.read_csv("data/data_extraction/entire_MP_data/AFLOW/AFLOW_keywords.txt", sep=",").columns)
    aflow_dict = {k: [] for k in keys}

    AFLOW_entries = get_dataframe_AFLOW(compound_list=compound_list, keys=keys, batch_size=1000, catalog="icsd",fileName="data/data_extraction/entire_MP_data/AFLOW/AFLOW_data.csv")

    #writing to csv
    AFLOW_entries.to_csv("data/data_extraction/entire_MP_data/AFLOW/AFLOW_data.csv", sep=",", index = False)
